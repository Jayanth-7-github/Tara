  ...
  async function handleLoad() {
    ...
    try {
      ...
      const sessionRecord = records.find(
        (r) => r.sessionName === targetSessionName,
      );
      ...
      if (sessionRecord) {
        setCurrentStatus({
          ...
          timestampText:
            sessionRecord.timestampText ||
            new Date(sessionRecord.timestamp).toLocaleString(),
        });
        setMessage("Student record loaded.");
      } else {
        // Not marked yet
        // If other records exist, get name, otherwise fetch from student API
        let name = "Unknown/New";
        if (records.length > 0) {
          name = records[0].name;
        } else {
          try {
            const stu = await fetchStudent(regno.trim());
            if (stu && stu.name) name = stu.name;
          } catch (e) {
            // ignore fetch error, fallback to Unknown
          }
        }
        setCurrentStatus({
          name,
        ...
        setMessage(
          "No attendance found for this session. You can mark it below.",
        );
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-950 via-black to-gray-900 text-white font-sans py-10 px-5">
      ...
      >
        ...
        <div className="p-8 space-y-6">
            ...
            <div className="bg-gray-800/40 p-4 rounded-lg border border-gray-700 animate-fade-in-up">
              ...
              {!currentStatus.isNew && currentStatus.isPresent && (
                <p className="text-xs text-gray-500 mt-2">
                  Time: {currentStatus.timestampText}
                </p>
              )}
            </div>
          )}
          ...
          {message && (
            <div
              className={`p-3 rounded text-sm ${message.toLowerCase().includes("absent") ? "bg-red-900/20 text-red-300 border border-red-900/50" : "bg-green-900/20 text-green-300 border border-green-900/50"}`}
            >
              {message}
            </div>
          )}
