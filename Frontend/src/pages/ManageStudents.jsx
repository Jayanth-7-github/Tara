...
export default function ManageStudents() {
  ...
  const normalizeBulkStudent = (input) => {
    const body = input && typeof input === "object" ? input : {};

    const originalKeys = Object.keys(body);
    const keyMap = {};
    for (const k of originalKeys) keyMap[String(k).toLowerCase().trim()] = k;
    ...
    const get = (...keys) => {
      for (const key of keys) {
        ...
        // case-insensitive
        const actual = keyMap[String(key).toLowerCase().trim()];
        if (
          actual &&
          body[actual] !== undefined &&
          body[actual] !== null &&
          String(body[actual]).trim() !== ""
        ) {
          return body[actual];
        }
      }
    ...
    const nameRaw = get("name", "Name");
    const emailRaw = get("email", "Email");
    const phoneRaw = get("phone", "Phone", "mobile", "Mobile");
    const departmentRaw = get("department", "Department", "branch", "Branch");
    const yearRaw = get("year", "Year");
    const teamNameRaw = get("teamName", "Team Name", "team name");
    const roleRaw = get("role", "Role");

    const normalized = {
      regno: regnoRaw != null ? String(regnoRaw).trim() : "",
      name: nameRaw != null ? String(nameRaw).trim() : "",
    };
    if (emailRaw != null) normalized.email = String(emailRaw).trim();
    if (phoneRaw != null) normalized.phone = String(phoneRaw).trim();
    if (departmentRaw != null)
      normalized.department = String(departmentRaw).trim();
    if (yearRaw != null) normalized.year = String(yearRaw).trim();
    if (teamNameRaw != null) normalized.teamName = String(teamNameRaw).trim();
    if (roleRaw != null) normalized.role = String(roleRaw).trim();
    return normalized;
  };
  ...
  const handleBulkCsvFile = (e) => {
    ...
    Papa.parse(file, {
      header: true,
      skipEmptyLines: true,
      transformHeader: (h) => (h != null ? String(h).trim() : h),
      complete: (results) => {
        try {
          ...
          const normalized = rows
            .map(normalizeBulkStudent)
            .filter(
              (r) => (r.regno && r.regno.trim()) || (r.name && r.name.trim()),
            );
          ...
          // If we have a specific SOLO event selected, drop teamName to keep payload clean
          const finalItems = normalized.map((r) => {
            const copy = { ...r };
            if (isSoloEvent) delete copy.teamName;
            return copy;
          });

          setBulkJson(JSON.stringify(finalItems, null, 2));
          showNotification(`Loaded ${finalItems.length} students from CSV`);
        } catch (err) {
          console.error("CSV normalization failed", err);
          showNotification("Failed to parse CSV", "error");
        }
      },
      error: (err) => {
        console.error("Papa.parse error", err);
        showNotification("Failed to read CSV", "error");
      },
    });
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      const items = rawItems.map(normalizeBulkStudent);

      if (isTeamEvent) {
        const missingTeam = items
          .map((stu, idx) => ({
            idx,
            regno: stu.regno,
        ...
        if (missingTeam.length > 0) {
          showNotification(
            `Team events require Team Name. Missing for ${missingTeam.length} student(s)`,
            "error",
          );
          return;
        }
      } else if (isSoloEvent) {
        // Solo event: teamName not required, and we keep CSV/UI clean
        items.forEach((s) => {
          delete s.teamName;
        });
      }
      ...
      if (selectedEventId) {
        try {
          ...
          showNotification(
            "Upload successful, but some registrations might have failed",
            "error",
          );
        }
      } else {
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-white group-hover:text-blue-400 transition-colors uppercase">
                              {student.name}
                            </div>
                            <div className="text-xs text-blue-400 font-mono mt-0.5">
                              {student.regno}
                            </div>
                            {isTeamEvent && student.teamName ? (
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          student.registrations.length > 0 ? (
                            student.registrations.slice(0, 2).map((reg, i) => (
                              <span
                                key={i}
                                className="text-[10px] bg-gray-700/50 border border-gray-600 text-gray-300 px-2 py-0.5 rounded-full"
                              >
                                {reg.eventName || "Event"}
                              </span>
                            ))
                          ...
                          {student.registrations &&
                            student.registrations.length > 2 && (
                              <span className="text-[10px] text-blue-400">
                                +{student.registrations.length - 2} more
                              </span>
                            )}
                        </div>
                      </td>
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                {!editingStudent && selectedEventId && (
                  <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl flex items-center justify-between">
                    ...
                    <div className="text-[10px] text-blue-400 font-medium bg-blue-400/10 px-2 py-0.5 rounded-full">
                      Register Mode
                    </div>
                  </div>
                )}
                {bulkMode ? (
                  <div className="space-y-4">
                    <p className="text-sm text-gray-400 mb-4">
                      Paste a JSON array of student objects. Required fields:{" "}
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="text-xs text-gray-500">
                        ...
                        <span className="font-mono text-gray-400">
                          Roll Number, Name, Email, Department, Year, Phone
                          {isTeamEvent ? ", Team Name" : ""}
                        </span>
                      </div>
                    </div>
                    ...
                    <textarea
                      ...
                      placeholder={
                        isTeamEvent
                          ? '[{"regno": "123", "name": "John", "teamName": "Team A"}, ...]'
                          : '[{"regno": "123", "name": "John"}, ...]'
                      }
