...
export default function StudentResults() {
      ...
      try {
        ...
        try {
          ...
          const eventTitles = new Set(
            visibleEvents.map((e) => (e.title || "").toLowerCase().trim()),
          );
  ...
  useEffect(() => {
    if (selectedEventTitle === "All") {
      ...
      // Filter results by eventName equal or testTitle starting with the event title
      const sel = selectedEventTitle.toLowerCase().trim();
      setFilteredResults(
        results.filter((r) => {
          const evName = (r.eventName || "").toLowerCase().trim();
          if (evName === sel) return true;
          const tt = (r.testTitle || "").toLowerCase().trim();
          return tt.startsWith(sel);
        }),
      );
    }
                ...
                {filteredResults.length === 0 ? (
                  ...
                  Object.values(
                    filteredResults.reduce((acc, result) => {
                      const userId =
                        result.userId?._id || result.userId?.id || "unknown";
                      if (!acc[userId]) {
                        acc[userId] = {
                          id: userId,
                          user: result.userId,
                      ...
                      if (
                        ...
                        new Date(acc[userId].latestDate)
                      ) {
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-wrap gap-2">
                              {group.results.map((result, idx) => {
                                ...
                                const isCoding = (result.testTitle || "")
                                  .toLowerCase()
                                  .includes("coding");
                                ...
                                const percentage =
                                  effectiveTotal > 0
                                    ? (result.score / effectiveTotal) * 100
                                    : 0;

                                return (
                                  <span
                                    key={idx}
                                    className={`px-2 py-1 rounded text-xs font-medium border border-opacity-20 ${
                                      percentage >= 70
                                        ...
                                        {Object.entries(result.answers).map(
                                          ([qId, ans], qIndex) => {
                                            ...
                                            return (
                                              <div
                                                key={qId}
                                                className="bg-gray-800/80 rounded-lg p-4 border border-gray-700/50 hover:border-blue-500/30 transition shadow-sm"
                                              >
