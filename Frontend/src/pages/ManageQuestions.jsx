...
export default function ManageQuestions() {
    ...
    const init = async () => {
      try {
        ...
        const myEvents = allEvents.filter(
          (e) => (e.managerEmail || "").toLowerCase().trim() === userEmail,
        );
        setEvents(myEvents);
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => {
    if (selectedEventId) {
      const ev = events.find(
        (e) => e._id === selectedEventId || e.id === selectedEventId,
      );
      if (ev) {
        setQuestions(ev.questions || []);
        setExamCode(ev.examSecurityCode || "");
      }
    } else {
      setQuestions([]);
      setExamCode("");
  ...
  const handleSaveQuestions = async () => {
    ...
    try {
      await updateEvent(selectedEventId, {
        questions,
        examSecurityCode: examCode,
      });
      setMessage({
        text: "Questions and exam code saved successfully!",
        type: "success",
      });
      updateLocalEvent(selectedEventId, questions, examCode);
    } catch (err) {
      console.error(err);
  ...
  const updateLocalEvent = (id, newQuestions, newExamCode) => {
    setEvents((prev) =>
      prev.map((e) =>
        e._id === id || e.id === id
          ? { ...e, questions: newQuestions, examSecurityCode: newExamCode }
          : e,
      ),
    );
  };
        ...
        <div className="bg-gray-800/50 p-6 rounded-xl border border-gray-700 mb-8">
          ...
          <div className="mt-4">
            ...
            <input
              ...
              value={examCode}
              onChange={(e) =>
                setExamCode(e.target.value.replace(/[^0-9]/g, ""))
              }
              className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none tracking-[0.3em] text-center"
              placeholder="e.g. 123456"
            />
            <p className="mt-1 text-xs text-gray-500">
              Students must enter this code on the exam page for this event.
              Leave blank to disable the code.
            </p>
          </div>
        </div>
        ...
        {selectedEventId && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              {message.text && (
                <div
                  className={`p-3 rounded ${message.type === "error" ? "bg-red-500/20 text-red-400" : "bg-green-500/20 text-green-400"}`}
                >
                  {message.text}
                </div>
              )}
              ...
              <div className="space-y-3 h-[600px] overflow-y-auto pr-2">
                ...
                {questions.map((q, idx) => (
                  <div
                    key={idx}
                    className="bg-gray-800 p-4 rounded-lg border border-gray-700 group hover:border-blue-500/50 transition relative"
                  >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded uppercase font-bold ${q.type === "coding" ? "bg-purple-900 text-purple-300" : "bg-blue-900 text-blue-300"}`}
                      >
                        {q.type}
                      </span>
                    ...
                    <p className="font-medium text-gray-200 line-clamp-2">
                      {q.text}
                    </p>
                  </div>
                ))}
                    ...
                    <div>
                      ...
                      {formData.testCases.map((tc, i) => (
                        <div key={i} className="flex gap-2 mb-2">
                          ...
                          <button
                            onClick={() => {
                              const newTCs = formData.testCases.filter(
                                (_, idx) => idx !== i,
                              );
                              setFormData({ ...formData, testCases: newTCs });
                            }}
